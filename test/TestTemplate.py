...
import unittest
from service_b.service_b_calc import ServiceBCalculation
...
class Test(unittest.TestCase):
    ...
    def setUp(self):
        CalculationServiceHelperFunctions.get_simulator_configuration_from_environment = (
            simulator_environment_e_connection
        )

    def test_example(self):
        # Arrange
        service = ServiceBCalculation()
        service.influx_connector = InfluxDBMock()
        pv_dispatch_params = {}
        pv_dispatch_params["Schedule"] = [3, 2, 1]
        service.init_calculation_service(None)

        # Execute
        ret_val = service.pv_dispatch(
            pv_dispatch_params, datetime(2024, 1, 1), TimeStepInformation(1, 2), "test-id", None
        )

        # Implement
        self.assertEqual(ret_val["PV_Dispatch"], 1.5)
        self.assertListEqual(
            [SimulaitonDataPoint("PV_Dispatch", datetime(2024, 1, 1), 1.5, "test-id")],
